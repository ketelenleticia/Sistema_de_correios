...
<head>
    ...
    <title>CorreiosSys - Encomendas</title>
        ...
        <main class="flex-1 flex flex-col">
            ...
            <div class="p-8 max-w-7xl w-full mx-auto flex-1">
                ...
                <div class="flex justify-between items-start mb-6">
                    <div>
                        <h1 class="text-2xl font-bold text-[#FFFFFF] mb-1">Encomendas</h1>
                        <p class="text-sm text-[#A1A1AA]">Gerencie as encomendas cadastradas no sistema.</p>
                    </div>
                    <a href="{{ route('encomendas.create') }}"
                ...
                <div class="bg-[#111111] rounded-2xl shadow-sm  border border-[#242424] overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-orange-500 text-white text-xs font-bold uppercase tracking-wider">
                                    <th class="py-3.5 px-4 w-16 text-center">ID</th>
                                    <th class="py-3.5 px-4">Descrição</th>
                                    <th class="py-3.5 px-4">Peso</th>
                                    <th class="py-3.5 px-4">Status</th>
                                    <th class="py-3.5 px-4">Data Envio</th>
                                    <th class="py-3.5 px-4">Data Entrega</th>
                                    <th class="py-3.5 px-4">Remetente</th>
                                    <th class="py-3.5 px-4">Destinatário</th>
                                    <th class="py-3.5 px-4 text-center w-40">Ações</th>
                                </tr>
                            </thead>

                            <tbody class="divide-y divide-[#242424] text-sm text-gray-700">
                                @foreach($encomendas as $encomenda)
                                <tr class="hover:bg-zinc-800/50 transition-colors duration-200">
                                    <td class="py-3.5 px-4 text-center font-medium text-[#FFFFFF]">
                                        {{ $encomenda->id }}
                                    </td>

                                    <td class="py-3.5 px-4 font-semibold text-[#A1A1AA] max-w-xs truncate">
                                        {{ $encomenda->descricao }}
                                    </td>

                                    <td class="py-3.5 px-4 text-[#A1A1AA] whitespace-nowrap">
                                        {{ $encomenda->peso }} kg
                                    </td>

                                    <td class="py-3.5 px-4 whitespace-nowrap">
                                        <span class="px-3 py-1 text-xs font-semibold rounded-full 
                                        @if($encomenda->status == 'Entregue') bg-emerald-900/50 text-emerald-400 border border-emerald-700
                                        @elseif($encomenda->status == 'Em trânsito') bg-orange-900/50 text-orange-400 border border-orange-700
                                        @elseif($encomenda->status == 'Pendente') bg-yellow-900/50  text-yellow-400 border border-yellow-700
                                    ...
                                    <td class="py-3.5 px-4 whitespace-nowrap text-[#A1A1AA]">
                                        {{ \Carbon\Carbon::parse($encomenda->data_envio)->format('d/m/Y') }}
                                    </td>
                                    ...
                                    <td class="py-3.5 px-4 text-[#A1A1AA] max-w-[120px] truncate">
                                        {{ $encomenda->remetente->nome}}
                                    </td>

                                    <td class="py-3.5 px-4 text-[#A1A1AA] max-w-[120px] truncate">
                                        {{ $encomenda->destinatario->nome  }}
                                    </td>

                                    <td class="py-3.5 px-4">
                                        <div class="flex justify-center gap-2">
                                            <a href="{{ route('encomendas.edit', $encomenda->id) }}"
                                                class="bg-blue-500 hover:bg-blue-600 text-white text-xs font-semibold py-1.5 px-3 rounded-lg inline-flex items-center gap-1 transition-colors">
                                                <i class="fa-solid fa-pen text-[10px]"></i> Editar
                                            </a>

                                            <form action="{{ route('encomendas.destroy', $encomenda->id) }}"
                                                ...
                                                <button type="submit"
                                                    ...
                                                    class="bg-red-500 hover:bg-red-600 text-white text-xs font-semibold py-1.5 px-3 rounded-lg inline-flex items-center gap-1 transition-colors">
                    ...
                    <div
                        ...
                        <div>
                            Mostrando {{ $encomendas->firstItem() }} a {{ $encomendas->lastItem() }} de
                            {{ $encomendas->total() }} registros
                        </div>

                        <div class="flex items-center gap-1">
                            {{-- Botão Voltar --}}
                            <a href="{{ $encomendas->previousPageUrl() }}"
                                class="w-8 h-8 flex items-center justify-center border  border-gray-500 rounded-lg hover:bg-gray-50 {{ $encomendas->onFirstPage() ? 'opacity-50 pointer-events-none' : '' }}">
                                <i class="fa-solid fa-chevron-left text-[10px]"></i>
                            </a>

                            {{-- Números das Páginas --}}
                            @foreach ($encomendas->getUrlRange(1, $encomendas->lastPage()) as $page => $url)
                            <a href="{{ $url }}"
                                class="w-8 h-8 flex items-center justify-center border  border-[#242424] rounded-lg font-semibold {{ $page == $encomendas->currentPage() ? 'bg-orange-500 text-white' : 'border border-gray-200 hover:bg-gray-50 text-gray-600' }}">
                                {{ $page }}
                            </a>
                            @endforeach
                            ...
                            <a href="{{ $encomendas->nextPageUrl() }}"
                                class="w-8 h-8 flex items-center justify-center border  border-gray-500 rounded-lg hover:bg-gray-50 {{ !$encomendas->hasMorePages() ? 'opacity-50 pointer-events-none' : '' }}">
