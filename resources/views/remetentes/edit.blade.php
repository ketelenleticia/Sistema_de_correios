...
<head>
    ...
    <title>Envios - Editar Remetente</title>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <main class="p-6 max-w-3xl w-full mx-auto flex-1 flex flex-col justify-start">
                ...
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h1 class="text-2xl font-bold text-[#FFFFFF] tracking-tight">Editar Remetente</h1>
                        <p class="text-sm text-[#A1A1AA] mt-1">Modifique os campos abaixo para atualizar as informações
                            do remetente no sistema.</p>
                    </div>
                    <a href="{{ route('remetentes.index') }}"
                ...
                <div class="bg-[#111111] rounded-2xl shadow-sm border border-[#242424]  p-6 md:p-8">
                    <form action="{{ route('remetentes.update', $remetente->id) }}" method="POST" class="space-y-6">
                        @method('PUT')
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div class="md:col-span-2 flex flex-col gap-1.5">
                                <label for="nome" class="text-xs font-bold text-[#A1A1AA] uppercase tracking-wider">Nome
                                    do Remetente</label>
                                <input type="text" name="nome" value="{{ $remetente->nome }}" required
                                    class="w-full bg-[#111111] border border-[#242424] rounded-xl px-4 py-3 text-sm text-[#A1A1AA] placeholder: focus:outline-none focus:border-orange-500">
                            </div>

                            <div class="flex flex-col gap-1.5">
                                <label for="cpf"
                                    class="text-xs font-bold text-[#A1A1AA] uppercase tracking-wider">CPF</label>
                                <input type="text" name="cpf" id="cpf" required value="{{ $remetente->cpf }}" required
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <input type="text" name="telefone" id="telefone" required
                                    value="{{ $remetente->telefone }}" required
                                    class="w-full bg-[#111111] border border-[#242424] rounded-xl px-4 py-3 text-sm text-[#A1A1AA] placeholder:text-[#4F4F4F] focus:outline-none focus:border-orange-500 ">
                            </div>
                            ...
                            <div class="md:col-span-2 flex flex-col gap-1.5">
                                <label for="endereco"
                                    ...
                                    Residencial</label>
                                <input type="text" name="endereco" id="endereco" value="{{ $remetente->endereco }}"
                                    required
                                    class="w-full bg-[#111111] border border-[#242424] rounded-xl px-4 py-3 text-sm text-[#A1A1AA] placeholder:text-[#4F4F4F] focus:outline-none focus:border-orange-500 ">
                            </div>
                        </div>
                        ...
                        <div class="border-t border-[#242424] pt-4 mt-6 flex justify-end">
                            <button type="submit"
                                ...
                                <i class="fa-solid fa-pen-to-square text-xs"></i> Atualizar Remetente
                            </button>
                        </div>
