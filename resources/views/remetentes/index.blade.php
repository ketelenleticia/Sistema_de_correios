...
<head>
    ...
    <title>CorreiosSys - Remetentes</title>
        ...
        <main class="flex-1 flex flex-col">
            ...
            <div class="p-8 max-w-7xl w-full mx-auto flex-1">
                ...
                <div class="flex justify-between items-start mb-6">
                    <div>
                        <h1 class="text-2xl font-bold text-[#FFFFFF] mb-1">Remetentes</h1>
                        <p class="text-sm text-[#A1A1AA]">Gerencie os remetentes cadastrados no sistema.</p>
                    </div>
                    <a href="{{ route('remetentes.create') }}"
                ...
                <div class="bg-[#111111] rounded-2xl shadow-sm border border-[#242424] overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-[#242424] text-sm">
                                @forelse($remetentes as $remetente)
                                <tr class="hover:bg-zinc-800/50 transition-colors duration-200">
                                    <td class="py-3.5 px-4 font-medium text-[#FFFFFF]">{{ $remetente->id }}</td>
                                    <td class="py-3.5 px-4 font-medium text-[#A1A1AA]">{{ $remetente->nome }}</td>
                                    <td class="py-3.5 px-4 text-[#A1A1AA]">{{ $remetente->cpf }}</td>
                                    <td class="py-3.5 px-4 text-[#A1A1AA]">{{ $remetente->telefone }}</td>
                                    <td class="py-3.5 px-4 max-w-xs truncate text-[#A1A1AA]">{{ $remetente->endereco }}
                                    </td>
                                    <td class="py-3.5 px-4">
                                        <div class="flex justify-center gap-2">
                                            <a href="{{ route('remetentes.edit', $remetente->id) }}"
                                            ...
                                            <form action="{{ route('remetentes.destroy', $remetente->id) }}"
                                ...
                                @empty
                                <tr>
                                    <td colspan="6" class="py-8 text-center text-gray-400 italic">Nenhum remetente
                                        cadastrado.</td>
                                </tr>
                                @endforelse
                            </tbody>
                    ...
                    <div
                        ...
                        <div>
                            Mostrando {{ $remetentes->firstItem() }} a {{ $remetentes->lastItem() }} de
                            {{ $remetentes->total() }} registros
                        </div>

                        <div class="flex items-center gap-1">
                            {{-- Botão Voltar --}}
                            <a href="{{ $remetentes->previousPageUrl() }}"
                                class="w-8 h-8 flex items-center justify-center border  border-gray-500 rounded-lg hover:bg-gray-50 {{ $remetentes->onFirstPage() ? 'opacity-50 pointer-events-none' : '' }}">
                                <i class="fa-solid fa-chevron-left text-[10px]"></i>
                            </a>

                            {{-- Números das Páginas --}}
                            @foreach ($remetentes->getUrlRange(1, $remetentes->lastPage()) as $page => $url)
                            <a href="{{ $url }}"
                                class="w-8 h-8 flex items-center justify-center border  border-[#242424] rounded-lg font-semibold {{ $page == $remetentes->currentPage() ? 'bg-orange-500 text-white' : 'border border-gray-200 hover:bg-gray-50 text-gray-600' }}">
                                {{ $page }}
                            </a>
                            @endforeach

                            {{-- Botão Avançar --}}
                            <a href="{{ $remetentes->nextPageUrl() }}"
                                class="w-8 h-8 flex items-center justify-center border  border-gray-500 rounded-lg hover:bg-gray-50 {{ !$remetentes->hasMorePages() ? 'opacity-50 pointer-events-none' : '' }}">
